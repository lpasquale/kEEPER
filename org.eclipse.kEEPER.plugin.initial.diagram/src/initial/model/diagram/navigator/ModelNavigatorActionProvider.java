	...
	private static class OpenDiagramAction extends Action {
		...
		*/
		public final void selectionChanged(IStructuredSelection selection) {
			...
			if (selection.size() == 1) {
				...
				if (selectedElement instanceof ModelNavigatorItem) {
					selectedElement = ((ModelNavigatorItem) selectedElement).getView();
				} else if (selectedElement instanceof IAdaptable) {
					selectedElement = ((IAdaptable) selectedElement).getAdapter(View.class);
				}
				if (selectedElement instanceof Diagram) {
					Diagram diagram = (Diagram) selectedElement;
					if (EnvironmentEditPart.MODEL_ID.equals(ModelVisualIDRegistry.getModelID(diagram))) {
						myDiagram = diagram;
					}
				}
			}
		...
		*/
		public void run() {
			...
			try {
				page.openEditor(editorInput, ModelDiagramEditor.ID);
			} catch (PartInitException e) {
				ModelDiagramEditorPlugin.getInstance().logError("Exception while openning diagram", e); //$NON-NLS-1$
			}
		}
