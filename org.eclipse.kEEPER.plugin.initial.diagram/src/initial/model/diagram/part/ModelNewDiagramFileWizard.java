...
public class ModelNewDiagramFileWizard extends Wizard {
	...
	*/
	public ModelNewDiagramFileWizard(URI domainModelURI, EObject diagramRoot,
		...
		diagramRootElementSelectionPage.setTitle(Messages.ModelNewDiagramFileWizard_RootSelectionPageTitle);
		diagramRootElementSelectionPage.setDescription(Messages.ModelNewDiagramFileWizard_RootSelectionPageDescription);
	...
	*/
	public boolean performFinish() {
		...
		ModelDiagramEditorUtil.setCharset(diagramFile);
		affectedFiles.add(diagramFile);
		URI diagramModelURI = URI.createPlatformResourceURI(diagramFile.getFullPath().toString(), true);
		ResourceSet resourceSet = myEditingDomain.getResourceSet();
		final Resource diagramResource = resourceSet.createResource(diagramModelURI);
		AbstractTransactionalCommand command = new AbstractTransactionalCommand(myEditingDomain,
				Messages.ModelNewDiagramFileWizard_InitDiagramCommand, affectedFiles) {
		...
		try {
			...
			diagramResource.save(ModelDiagramEditorUtil.getSaveOptions());
			ModelDiagramEditorUtil.openDiagram(diagramResource);
		} catch (ExecutionException e) {
			ModelDiagramEditorPlugin.getInstance().logError("Unable to create model and diagram", e); //$NON-NLS-1$
		} catch (IOException ex) {
			ModelDiagramEditorPlugin.getInstance().logError("Save operation failed for: " + diagramModelURI, ex); //$NON-NLS-1$
		} catch (PartInitException ex) {
			ModelDiagramEditorPlugin.getInstance().logError("Unable to open editor", ex); //$NON-NLS-1$
		}
		return true;
	}
	...
	private static class DiagramRootElementSelectionPage extends ModelElementSelectionPage {
		...
		*/
		protected String getSelectionTitle() {
			return Messages.ModelNewDiagramFileWizard_RootSelectionPageSelectionTitle;
		}
		...
		*/
		protected boolean validatePage() {
			if (getModelElement() == null) {
				setErrorMessage(Messages.ModelNewDiagramFileWizard_RootSelectionPageNoSelectionMessage);
				return false;
			}
