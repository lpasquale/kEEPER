...
public class ModelBaseItemSemanticEditPolicy extends SemanticEditPolicy {
	...
	public Command getCommand(Request request) {
		if (request instanceof ReconnectRequest) {
			...
			if (view instanceof View) {
				Integer id = new Integer(ModelVisualIDRegistry.getVisualID((View) view));
	...
	*/
	private IElementType getContextElementType(IEditCommandRequest request) {
		IElementType requestContextElementType = ModelElementTypes.getElementType(getVisualID(request));
	...
	*/
	public static LinkConstraints getLinkConstraints() {
		LinkConstraints cached = ModelDiagramEditorPlugin.getInstance().getLinkConstraints();
		if (cached == null) {
			ModelDiagramEditorPlugin.getInstance().setLinkConstraints(cached = new LinkConstraints());
		}
		return cached;
	}
	...
	public static class LinkConstraints {
		...
		*/
		...
		public boolean canCreatePrimitiveEventAgent_4001(PrimitiveEvent source, Agent target) {
			...
			return canExistPrimitiveEventAgent_4001(source, target);
		}
		...
		public boolean canCreatePrimitiveEventObserver_4002(PrimitiveEvent source, Observer target) {
			...
			return canExistPrimitiveEventObserver_4002(source, target);
		}
