...
public class EnvironmentCanonicalEditPolicy extends CanonicalEditPolicy {
	...
	*/
	protected void refreshSemantic() {
		...
		for (Iterator<ModelNodeDescriptor> descriptorsIterator = childDescriptors.iterator(); descriptorsIterator
				.hasNext();) {
	...
	*/
	private Collection<IAdaptable> refreshConnections() {
		...
		Collection<ModelLinkDescriptor> linkDescriptors = collectAllLinks(getDiagram(), domain2NotationMap);
		Collection existingLinks = new LinkedList(getDiagram().getEdges());
		for (Iterator linksIterator = existingLinks.iterator(); linksIterator.hasNext();) {
			Edge nextDiagramLink = (Edge) linksIterator.next();
			int diagramLinkVisualID = ModelVisualIDRegistry.getVisualID(nextDiagramLink);
			...
			for (Iterator<ModelLinkDescriptor> linkDescriptorsIterator = linkDescriptors
				...
				ModelLinkDescriptor nextLinkDescriptor = linkDescriptorsIterator.next();
	...
	* @generated
	*/
	private Collection<ModelLinkDescriptor> collectAllLinks(View view, Domain2Notation domain2NotationMap) {
		if (!EnvironmentEditPart.MODEL_ID.equals(ModelVisualIDRegistry.getModelID(view))) {
		...
		case EnvironmentEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(ModelDiagramUpdater.getEnvironment_1000ContainedLinks(view));
			}
		...
		case InstanceEditPart.VISUAL_ID: {
			if (!domain2NotationMap.containsKey(view.getElement())) {
				result.addAll(ModelDiagramUpdater.getInstance_2005ContainedLinks(view));
			}
			domain2NotationMap.putView(view.getElement(), view);
			break;
		}
	...
	* @generated
	*/
	private Collection<IAdaptable> createConnections(Collection<ModelLinkDescriptor> linkDescriptors,
		...
		for (ModelLinkDescriptor nextLinkDescriptor : linkDescriptors) {
			...
			CreateConnectionViewRequest.ConnectionViewDescriptor descriptor = new CreateConnectionViewRequest.ConnectionViewDescriptor(
					...
					ModelVisualIDRegistry.getType(nextLinkDescriptor.getVisualID()), ViewUtil.APPEND, false,
